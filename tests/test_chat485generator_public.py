"""Public unit tests for chat485generator.
...
def test_help():
    """Verify chat485generator --help output."""
...
def test_output(tmpdir):
    """Verify chat485generator --output changes output dir.
    ...
    # Run chat485generator using the --output option
    subprocess.run(
        ["chat485generator", "--output", "myout", "hello"],
        check=True,
        cwd=tmpdir,
    )

    # Verify files are present in "myout/" directory
    assert (tmpdir / "myout").exists()
    assert (tmpdir / "myout/index.html").exists()


def test_hello(tmpdir):
    """Test chat485generator with published "hello" input.
    ...
    output_dir = tmpdir / "generated_html"
    index_path = output_dir / "index.html"

    if output_dir.exists():
        shutil.rmtree(output_dir)

    # Run chat485generator in tmpdir
    subprocess.run(["chat485generator", "hello"], check=True, cwd=tmpdir)
...
def test_hello_css(tmpdir):
    """Test chat485generator with published "hello_css" input.
    ...
    # Run chat485generator in tmpdir
    subprocess.run(["chat485generator", "hello_css"], check=True, cwd=tmpdir)
